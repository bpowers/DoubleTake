...
public:
  ...
  void recordSyncEvent(thrSyncCmd synccmd, int ret) {
    ...
    PRINF("recordSyncEvent line %d: event %p (at %ld) thread %d eventlist %p", __LINE__, event, current->syncevents.getEntriesNumb()-1, current->index, this);
  ...
  struct syncEvent* prepareRollback() {
    ...
    PRINF("prepareRollback: synceventlist at %p event %p", &list, event);
    return event;
  }
